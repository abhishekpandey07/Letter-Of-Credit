...
const bankDB = mongoose.model('banks');
  ...
  .get(function (req, res) {
    ...
    LCDB.aggregate([{
      ...
      if (error) {
        ...
        res.json(JSON.stringify(dueThisMonth))
      }
    })
  ...
  LCDB.aggregate([{
    ...
    },
    {
      $group: {
        _id: {
          month: {
            $month: "$dueDetails.due_DT"
          },
          year: {
            $year: "$dueDetails.due_DT"
          }
        },
        dueAmount: {
          $sum: "$dueDetails.due_amt"
        },
...
router.route('/month').get(function (req, res) {
  LCDB.aggregate([{
    ...
    {
      $sort: {
        "payment.cycles.due_DT": 1
      }
    },
    {
      $group: {
        _id: {
          issuer: "$issuingBank.name",
          month: {
            $month: "$payment.cycles.due_DT"
          },
          year: {
            $year: "$payment.cycles.due_DT"
          }
        },
        amount: {
          $sum: "$payment.cycles.due_amt"
        },
        LC: {
          $push: {
            supplier: "$supplierd.name",
  ...
  LCDB.aggregate([{
    ...
    },
    {
      $project: {
        ...
        amount: 1,
        unUtilized: {
          $subtract: ["$amount", "$payment.total_due"]
        },
